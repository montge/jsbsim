...
/*******************************************************************************
 * C172x Integration Tests for FGKinemat
 *
 * Tests kinematic component behavior using the C172x model.
 * Verifies rate-limited actuator movement and detent positions.
 ******************************************************************************/
class FGKinematC172xTest : public CxxTest::TestSuite
{
public:
  // Test 1: C172x FCS loads with kinematic components
  void testC172xFCSLoadsWithKinematics() {
    FGFDMExec fdmex;
  ...
  // Test 2: Flap kinematic movement (detents)
  void testC172xFlapKinematics() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    ...
    // Flaps take time to extend
    double lastPos = 0.0;
    for (int i = 0; i < 500; i++) {
      fdmex.Run();
      double pos = fdmex.GetPropertyValue("fcs/flap-pos-deg");
      TS_ASSERT(std::isfinite(pos));
      TS_ASSERT(pos >= lastPos - 0.1);  // Generally increasing
      lastPos = pos;
    }

    // Should have extended
    TS_ASSERT(lastPos > 0.0);
  }

  // Test 3: Flap retraction
  void testC172xFlapRetraction() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
    ...
    double extendedPos = fdmex.GetPropertyValue("fcs/flap-pos-deg");

    // Retract flaps
    fcs->SetDfCmd(0.0);
    for (int i = 0; i < 300; i++) fdmex.Run();

    double retractedPos = fdmex.GetPropertyValue("fcs/flap-pos-deg");

    TS_ASSERT(extendedPos > retractedPos);
    TS_ASSERT_DELTA(retractedPos, 0.0, 1.0);
  }

  // Test 4: Flap detent positions
  void testC172xFlapDetents() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
    ...
    // C172x has 0, 10, 20, 30 degree detents
    double flapSettings[] = {0.0, 0.33, 0.66, 1.0};
    double expectedDeg[] = {0.0, 10.0, 20.0, 30.0};

    for (int j = 0; j < 4; j++) {
      fcs->SetDfCmd(flapSettings[j]);
      for (int i = 0; i < 300; i++) fdmex.Run();

      double pos = fdmex.GetPropertyValue("fcs/flap-pos-deg");
      TS_ASSERT(std::isfinite(pos));
      TS_ASSERT_DELTA(pos, expectedDeg[j], 2.0);
    }
  }

  // Test 5: Rate-limited elevator actuator
  void testC172xElevatorActuatorRate() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
  ...
  // Test 6: Aileron actuator rate limiting
  void testC172xAileronActuatorRate() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
    ...
    for (int i = 0; i < 50; i++) {
      fdmex.Run();
      double pos = fdmex.GetPropertyValue("fcs/left-aileron-pos-rad");
      TS_ASSERT(std::isfinite(pos));
      // Rate should be limited (no huge jumps)
      TS_ASSERT(std::abs(pos - lastPos) < 0.5);
      lastPos = pos;
    }
  }

  // Test 7: Continuous control surface movement
  void testC172xContinuousMovement() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
    ...
    // Sweep command continuously
    for (double cmd = -1.0; cmd <= 1.0; cmd += 0.1) {
      fcs->SetDeCmd(cmd);
  ...
  // Test 8: Kinematic position tracking
  void testC172xPositionTracking() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
    ...
    for (int i = 0; i < 100; i++) {
      fdmex.Run();
      lastPos = pos;
      pos = fdmex.GetPropertyValue("fcs/elevator-pos-rad");
    }

    // Should have reached steady state
    TS_ASSERT(std::abs(pos - lastPos) < 0.001);
  }

  // Test 9: Flap position normalized output
  void testC172xFlapNormalizedOutput() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
    ...
    TS_ASSERT(flapNorm >= 0.0 && flapNorm <= 1.0);
  }

  // Test 10: Multiple kinematic components running together
  void testC172xMultipleKinematicComponents() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
    ...
    TS_ASSERT(std::isfinite(fdmex.GetPropertyValue("fcs/flap-pos-deg")));
  }

  // Test 11: Actuator response during flight
  void testC172xActuatorResponseInFlight() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
  ...
  // Test 12: Flap intermediate position stop
  void testC172xFlapIntermediateStop() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
    ...
    // Command flaps to 10 degrees
    fcs->SetDfCmd(0.33);
    for (int i = 0; i < 200; i++) fdmex.Run();

    double pos10 = fdmex.GetPropertyValue("fcs/flap-pos-deg");
    TS_ASSERT_DELTA(pos10, 10.0, 2.0);

    // Command to 20 degrees
    fcs->SetDfCmd(0.66);
    for (int i = 0; i < 200; i++) fdmex.Run();

    double pos20 = fdmex.GetPropertyValue("fcs/flap-pos-deg");
    TS_ASSERT_DELTA(pos20, 20.0, 2.0);
    TS_ASSERT(pos20 > pos10);
  }
};
