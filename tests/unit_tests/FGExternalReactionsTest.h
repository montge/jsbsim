...
//=============================================================================
// C172x Integration Tests - External Reactions with Aircraft Model
//=============================================================================

class FGExternalReactionsC172xTest : public CxxTest::TestSuite
{
private:
...
public:
  ...
  // Test 1: External reactions model exists
  void testExternalReactionsExists() {
    auto extReact = fdm.GetExternalReactions();
    TS_ASSERT(extReact != nullptr);
  }

  // Test 2: External forces initially zero
  void testExternalForcesZero() {
    auto extReact = fdm.GetExternalReactions();
    const FGColumnVector3& forces = extReact->GetForces();
  ...
  // Test 3: External moments initially zero
  void testExternalMomentsZero() {
    auto extReact = fdm.GetExternalReactions();
    const FGColumnVector3& moments = extReact->GetMoments();
  ...
  // Test 4: External reactions during flight
  void testExternalReactionsDuringFlight() {
    auto ic = fdm.GetIC();
  ...
  // Test 5: External reactions after init
  void testExternalReactionsAfterInit() {
    auto extReact = fdm.GetExternalReactions();
    bool result = extReact->InitModel();
  ...
  // Test 6: Aircraft total forces include external
  void testAircraftTotalForces() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
    ...
    auto extReact = fdm.GetExternalReactions();
    const FGColumnVector3& extForces = extReact->GetForces();

    // External forces should be finite even if zero
    TS_ASSERT(std::isfinite(extForces(1)));
    TS_ASSERT(std::isfinite(extForces(2)));
    TS_ASSERT(std::isfinite(extForces(3)));
  }

  // Test 7: Run model with C172x
  void testRunModelWithC172x() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
    ...
    TS_ASSERT_EQUALS(result, false);  // False means success (no holding)
  }

  // Test 8: Forces remain zero without external inputs
  void testForcesRemainZeroWithoutInput() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
    ...
    const FGColumnVector3& forces = extReact->GetForces();
    // Without external force definitions, should remain zero
    TS_ASSERT_DELTA(forces.Magnitude(), 0.0, 0.01);
  }

  // Test 9: Moments remain zero without external inputs
  void testMomentsRemainZeroWithoutInput() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
    ...
    const FGColumnVector3& moments = extReact->GetMoments();
    TS_ASSERT_DELTA(moments.Magnitude(), 0.0, 0.01);
  }

  // Test 10: Vector access consistency
  void testVectorAccessConsistency() {
    auto extReact = fdm.GetExternalReactions();
  ...
  // Test 11: Model initialization multiple times
  void testModelInitMultipleTimes() {
    auto extReact = fdm.GetExternalReactions();
