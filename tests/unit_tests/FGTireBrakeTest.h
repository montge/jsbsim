...
/*******************************************************************************
 * FGTireBrake C172x Integration Tests
 * Tests tire and brake behavior using real C172x aircraft
 ******************************************************************************/
class FGTireBrakeC172xTest : public CxxTest::TestSuite
{
public:
  ...
  // Test C172x left brake application
  void testC172xLeftBrake() {
    ...
    // Left brake should cause left (negative) yaw rate
    TS_ASSERT(std::isfinite(yawRate));
  }

  // Test C172x right brake application
  void testC172xRightBrake() {
    ...
    // Right brake should cause right (positive) yaw rate
    TS_ASSERT(std::isfinite(yawRate));
  }

  // Test C172x symmetrical braking
  void testC172xSymmetricalBraking() {
    TS_ASSERT(fdmex.LoadModel("c172x"));
    auto ic = fdmex.GetIC();
    ic->SetAltitudeAGLFtIC(0.0);
  ...
  // Test C172x brake modulation
  void testC172xBrakeModulation() {
    TS_ASSERT(fdmex.LoadModel("c172x"));
    auto ic = fdmex.GetIC();
    ic->SetAltitudeAGLFtIC(0.0);
    ic->SetVgroundKtsIC(30.0);
    ...
    auto propagate = fdmex.GetPropagate();
    double speed1 = propagate->GetVel().Magnitude();
    ...
    double speed2 = propagate->GetVel().Magnitude();
    ...
    double speed3 = propagate->GetVel().Magnitude();

    // Verify deceleration occurred
    TS_ASSERT(speed2 < speed1);
    TS_ASSERT(speed3 < speed2);
  }

  // Test C172x brake release
  void testC172xBrakeRelease() {
    TS_ASSERT(fdmex.LoadModel("c172x"));
    auto ic = fdmex.GetIC();
    ic->SetAltitudeAGLFtIC(0.0);
    ic->SetVgroundKtsIC(20.0);
    TS_ASSERT(fdmex.RunIC());
    ...
    // Aircraft should still be moving (not stopped yet)
    auto propagate = fdmex.GetPropagate();
    TS_ASSERT(std::isfinite(propagate->GetVel().Magnitude()));
  }

  // Test C172x tire ground reaction
  void testC172xTireGroundReaction() {
    TS_ASSERT(fdmex.LoadModel("c172x"));
    auto ic = fdmex.GetIC();
    ic->SetAltitudeAGLFtIC(0.0);
    ...
    // Verify tires have ground reaction
    bool hasForce = false;
    for (int i = 0; i < gr->GetNumGearUnits(); i++) {
      auto gear = gr->GetGearUnit(i);
      if (gear->GetWOW()) {
        auto forces = gear->GetBodyForces();
  ...
  // Test C172x rolling on ground
  void testC172xRollingOnGround() {
    TS_ASSERT(fdmex.LoadModel("c172x"));
    auto ic = fdmex.GetIC();
    ic->SetAltitudeAGLFtIC(0.0);
    ic->SetVgroundKtsIC(20.0);
  ...
  // Test C172x brake effectiveness at speed
  void testC172xBrakeEffectivenessAtSpeed() {
    TS_ASSERT(fdmex.LoadModel("c172x"));
    auto ic = fdmex.GetIC();
    ic->SetAltitudeAGLFtIC(0.0);
    ic->SetVgroundKtsIC(50.0);  // Higher speed
    TS_ASSERT(fdmex.RunIC());
    ...
    // Brakes should significantly reduce speed
    TS_ASSERT(finalSpeed < initialSpeed * 0.8);
  }

  // Test C172x ground roll stability with brakes
  void testC172xBrakingStability() {
    TS_ASSERT(fdmex.LoadModel("c172x"));
    auto ic = fdmex.GetIC();
    ic->SetAltitudeAGLFtIC(0.0);
    ic->SetVgroundKtsIC(30.0);
    ...
    // Verify stability during braking
    for (int i = 0; i < 30; i++) {
      fdmex.Run();
      ...
      // Should remain stable during braking
      TS_ASSERT(fabs(roll) < 0.3);
      TS_ASSERT(fabs(pitch) < 0.3);
    }
  }

  // Test C172x parking brake simulation
  void testC172xParkingBrakeSimulation() {
    TS_ASSERT(fdmex.LoadModel("c172x"));
    auto ic = fdmex.GetIC();
    ic->SetAltitudeAGLFtIC(0.0);
    ic->SetVgroundKtsIC(0.0);  // Parked
    TS_ASSERT(fdmex.RunIC());

    auto fcs = fdmex.GetFCS();
    fcs->SetLBrake(1.0);  // Simulate parking brake
    fcs->SetRBrake(1.0);
    ...
    // Aircraft should remain stationary
    TS_ASSERT(speed < 1.0);  // Near zero
  }

  // Test C172x tire friction during landing
  void testC172xLandingTireFriction() {
    TS_ASSERT(fdmex.LoadModel("c172x"));
    auto ic = fdmex.GetIC();
    ic->SetAltitudeAGLFtIC(5.0);  // Just above ground
    ic->SetVcalibratedKtsIC(60.0);
    ic->SetClimbRateFpsIC(-3.0);  // Descending
    TS_ASSERT(fdmex.RunIC());
    ...
    for (int i = 0; i < 100; i++) {
      ...
      for (int j = 0; j < gr->GetNumGearUnits(); j++) {
        if (gr->GetGearUnit(j)->GetWOW()) {
          // After touchdown, check friction forces
