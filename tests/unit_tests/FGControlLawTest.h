...
class FGControlLawC172xTest : public CxxTest::TestSuite
{
public:
  void testC172xElevatorControl() {
    // Test elevator control law response
    FGFDMExec fdmex;
  ...
  void testC172xAileronControl() {
    // Test aileron control law response
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
  ...
  void testC172xRudderControl() {
    // Test rudder control law response
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
  ...
  void testC172xControlSurfaceLimits() {
    ...
    double dePos = fcs->GetDePos();
    TS_ASSERT(std::isfinite(dePos));
    // Should be limited to physical range
  }

  void testC172xSpeedbrake() {
    // Test speedbrake (may be zero for C172x)
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
  ...
  void testC172xDynamicPressure() {
    // Test dynamic pressure affects control effectiveness
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
  ...
  void testC172xControlResponse() {
    // Test control surface response time
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    fdmex.Run();
    double afterOneStep = fcs->GetDePos();

    for (int i = 0; i < 20; i++) {
      fdmex.Run();
    }
    double steadyState = fcs->GetDePos();

    TS_ASSERT(std::isfinite(initialPos));
    TS_ASSERT(std::isfinite(afterOneStep));
    TS_ASSERT(std::isfinite(steadyState));
  }

  void testC172xTrimControl() {
    // Test trim tab or trim wheel
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
