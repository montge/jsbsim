...
// ============ C172x Aircraft Thermodynamics Integration Tests ============
class FGThermodynamicsC172xTest : public CxxTest::TestSuite
{
public:

  // Test C172x atmospheric temperature access
  void testC172xAtmosphericTemperature() {
    FGFDMExec fdmex;
  ...
  // Test C172x engine temperature effects
  void testC172xEngineTemperatureEffects() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
  ...
  // Test C172x density at various altitudes
  void testC172xDensityVsAltitude() {
    double densities[4];
    double alts[] = {0.0, 3000.0, 6000.0, 9000.0};
    ...
    // Verify decreasing density with altitude
    for (int i = 1; i < 4; i++) {
      TS_ASSERT(densities[i] < densities[i-1]);
    }
  }

  // Test C172x pressure ratio
  void testC172xPressureRatio() {
    ...
    auto atm = fdmex.GetAtmosphere();
    double pressRatio = atm->GetPressureRatio();

    TS_ASSERT(std::isfinite(pressRatio));
    TS_ASSERT(pressRatio > 0.0);
    TS_ASSERT(pressRatio <= 1.0);
  }

  // Test C172x temperature ratio
  void testC172xTemperatureRatio() {
    ...
    auto atm = fdmex.GetAtmosphere();
    double tempRatio = atm->GetTemperatureRatio();

    TS_ASSERT(std::isfinite(tempRatio));
    TS_ASSERT(tempRatio > 0.0);
    TS_ASSERT(tempRatio <= 1.0);
  }

  // Test C172x total temperature
  ...
  // Test C172x stagnation properties
  void testC172xStagnationProperties() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    ...
    // Total values >= static values
    TS_ASSERT(TAT >= SAT);
    TS_ASSERT(pt >= ps);
  }

  // Test C172x engine fuel consumption thermodynamics
  void testC172xEngineFuelConsumption() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    ...
    auto engine = prop->GetEngine(0);
    double fuelFlow = engine->GetFuelFlowRate();

    TS_ASSERT(std::isfinite(fuelFlow));
    // Fuel flow should be positive when engine running
  }

  // Test C172x atmospheric properties stability
  void testC172xAtmosphericPropertiesStability() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    ...
    double prevTemp = atm->GetTemperature();
    double prevPress = atm->GetPressure();
    ...
    for (int i = 0; i < 200; i++) {
      ...
      if (i > 10) {
        TS_ASSERT(std::abs(temp - prevTemp) < 20.0);
        TS_ASSERT(std::abs(press - prevPress) < 100.0);
      }
  ...
  // Test C172x ideal gas relationship
  void testC172xIdealGasRelationship() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    ...
    auto atm = fdmex.GetAtmosphere();
    double temp = atm->GetTemperature();    // Rankine
    double press = atm->GetPressure();      // psf
    double dens = atm->GetDensity();        // slug/ft^3
    ...
    // P = rho * R * T (ideal gas)
    // Check that these values are physically consistent
  }

  // Test C172x speed of sound thermodynamics
  void testC172xSpeedOfSoundThermodynamics() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    ...
    // a = sqrt(gamma * R * T)
    // Sound speed should be related to temperature
    TS_ASSERT(soundSpeed > 800.0);   // fps
    TS_ASSERT(soundSpeed < 1200.0);  // fps
  }

  // Test C172x kinematic viscosity thermodynamics
  void testC172xKinematicViscosity() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    ...
    auto atm = fdmex.GetAtmosphere();
    double kinVisc = atm->GetKinematicViscosity();
    double absVisc = atm->GetAbsoluteViscosity();
    double dens = atm->GetDensity();

    TS_ASSERT(std::isfinite(kinVisc));
    TS_ASSERT(std::isfinite(absVisc));
    TS_ASSERT(kinVisc > 0.0);
    TS_ASSERT(absVisc > 0.0);

    // nu = mu / rho
    if (dens > 0.0) {
      double calculatedKinVisc = absVisc / dens;
      // Should be reasonably close
      TS_ASSERT(std::abs(kinVisc - calculatedKinVisc) / kinVisc < 0.01);
    }
  }
};
