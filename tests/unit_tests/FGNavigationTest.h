...
//=============================================================================
// C172x Integration Tests - Navigation and Position Tests
//=============================================================================

class FGNavigationC172xTest : public CxxTest::TestSuite
{
private:
...
public:
  ...
  // Test 1: Initial position is valid
  void testInitialPositionValid() {
    auto ic = fdm.GetIC();
    ...
    double lat = prop->GetLatitudeDeg();
    double lon = prop->GetLongitudeDeg();

    TS_ASSERT(std::isfinite(lat));
    TS_ASSERT(std::isfinite(lon));
    TS_ASSERT_DELTA(lat, 40.0, 0.1);
    TS_ASSERT_DELTA(lon, -74.0, 0.1);
  }

  // Test 2: Altitude tracking
  void testAltitudeTracking() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
    ...
    double altitude = prop->GetAltitudeASL();
    TS_ASSERT(std::isfinite(altitude));
    TS_ASSERT_DELTA(altitude, 5000.0, 100.0);
  }

  // Test 3: Ground track heading
  void testGroundTrackHeading() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    ic->SetPsiDegIC(90.0);  // East
    fdm.RunIC();

    for (int i = 0; i < 20; ++i) fdm.Run();

    auto aux = fdm.GetAuxiliary();
    double track = aux->GetGroundTrack() * RAD_TO_DEG;

    TS_ASSERT(std::isfinite(track));
  }

  // Test 4: Position changes during flight
  void testPositionChangesDuringFlight() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    ic->SetLatitudeDegIC(40.0);
    ...
    auto prop = fdm.GetPropagate();
    double initialLon = prop->GetLongitudeDeg();

    // Run for some time
    for (int i = 0; i < 500; ++i) fdm.Run();

    double finalLon = prop->GetLongitudeDeg();

    // Heading east, longitude should increase
    TS_ASSERT(std::isfinite(finalLon));
  }

  // Test 5: Ground speed calculation
  ...
  void testEulerAngles() {
    ...
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    ic->SetPhiDegIC(10.0);   // Bank
    ic->SetThetaDegIC(5.0);  // Pitch
    ic->SetPsiDegIC(45.0);   // Heading
    fdm.RunIC();

    auto prop = fdm.GetPropagate();

    double phi = prop->GetEuler(1) * RAD_TO_DEG;
    double theta = prop->GetEuler(2) * RAD_TO_DEG;
    double psi = prop->GetEuler(3) * RAD_TO_DEG;
  ...
  void testVelocityComponents() {
    ...
    auto prop = fdm.GetPropagate();
    const FGColumnVector3& vUVW = prop->GetUVW();

    TS_ASSERT(std::isfinite(vUVW(1)));
    TS_ASSERT(std::isfinite(vUVW(2)));
    TS_ASSERT(std::isfinite(vUVW(3)));
  }

  // Test 8: Angular rates
  void testAngularRates() {
    ...
    auto fcs = fdm.GetFCS();
    fcs->SetDaCmd(0.2);  // Roll input

    for (int i = 0; i < 50; ++i) fdm.Run();

    auto prop = fdm.GetPropagate();
    const FGColumnVector3& pqr = prop->GetPQR();

    TS_ASSERT(std::isfinite(pqr(1)));  // Roll rate
    TS_ASSERT(std::isfinite(pqr(2)));  // Pitch rate
    TS_ASSERT(std::isfinite(pqr(3)));  // Yaw rate
  }

  // Test 9: Location in ECEF
  void testLocationECEF() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    ic->SetLatitudeDegIC(40.0);
  ...
  // Test 10: Radius from Earth center
  void testRadiusFromEarthCenter() {
    ...
    TS_ASSERT(radius > 20000000.0);  // > 20 million feet
    TS_ASSERT(std::isfinite(radius));
  }

  // Test 11: Geodetic vs geocentric latitude
  void testGeodeticLatitude() {
    ...
    auto prop = fdm.GetPropagate();
    double geodeticLat = prop->GetGeodLatitudeDeg();
    double geocentricLat = prop->GetLatitudeDeg();

    TS_ASSERT(std::isfinite(geodeticLat));
    TS_ASSERT(std::isfinite(geocentricLat));
  }

  // Test 12: Extended simulation navigation stability
  void testExtendedSimulationNavStability() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    ic->SetLatitudeDegIC(40.0);
    ...
    TS_ASSERT(std::isfinite(prop->GetEuler(1)));
    TS_ASSERT(std::isfinite(prop->GetEuler(2)));
    TS_ASSERT(std::isfinite(prop->GetEuler(3)));
  }
};
