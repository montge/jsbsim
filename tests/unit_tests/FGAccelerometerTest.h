...
/*******************************************************************************
 * C172x Integration Tests for Accelerometer/Acceleration Sensing
 ******************************************************************************/

class FGAccelerometerC172xTest : public CxxTest::TestSuite
{
public:
  // Test body-axis X acceleration measurement
  void testC172xBodyXAcceleration() {
    FGFDMExec fdmex;
  ...
  // Test body-axis Y acceleration measurement
  void testC172xBodyYAcceleration() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    TS_ASSERT(std::isfinite(ay));
  }

  // Test body-axis Z acceleration measurement
  void testC172xBodyZAcceleration() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    TS_ASSERT(std::isfinite(az));
  }

  // Test gravity sensed at rest
  void testC172xGravityAtRest() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    // At rest on ground, Z accel should be around -g (negative = upward)
    double az = pm->GetNode("accelerations/a-pilot-z-ft_sec2", true)->getDoubleValue();
    TS_ASSERT(std::isfinite(az));
  }

  // Test forward acceleration during takeoff
  void testC172xForwardAccelerationTakeoff() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    double ax = accel->GetBodyAccel(1);
    TS_ASSERT(std::isfinite(ax));
    // Should have positive forward acceleration with full power
    TS_ASSERT(ax > 0.0);
  }

  // Test load factor during level flight
  void testC172xLoadFactorLevelFlight() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    // Load factor should be positive (around 1g in level flight)
    TS_ASSERT(nz > 0.0);
  }

  // Test load factor during pullup
  void testC172xLoadFactorPullup() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    double pullup_nz = aux->GetNz();

    TS_ASSERT(std::isfinite(baseline_nz));
    TS_ASSERT(std::isfinite(pullup_nz));
  }

  // Test lateral acceleration in sideslip
  void testC172xLateralAccelerationSideslip() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    // Apply rudder without aileron (uncoordinated)
    fcs->SetDrCmd(0.5);
  ...
  // Test acceleration via property tree
  void testC172xAccelerationViaPropertyTree() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    double udot = pm->GetNode("accelerations/udot-ft_sec2", true)->getDoubleValue();
    double vdot = pm->GetNode("accelerations/vdot-ft_sec2", true)->getDoubleValue();
    double wdot = pm->GetNode("accelerations/wdot-ft_sec2", true)->getDoubleValue();
  ...
  // Test pilot acceleration (includes gravity)
  void testC172xPilotAcceleration() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    double ax_pilot = pm->GetNode("accelerations/a-pilot-x-ft_sec2", true)->getDoubleValue();
    double ay_pilot = pm->GetNode("accelerations/a-pilot-y-ft_sec2", true)->getDoubleValue();
    double az_pilot = pm->GetNode("accelerations/a-pilot-z-ft_sec2", true)->getDoubleValue();

    TS_ASSERT(std::isfinite(ax_pilot));
    TS_ASSERT(std::isfinite(ay_pilot));
    TS_ASSERT(std::isfinite(az_pilot));
  }

  // Test acceleration stability during cruise
  void testC172xAccelerationStabilityCruise() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    for (int i = 0; i < 1000; i++) {
      fdmex.Run();
      max_ax = std::max(max_ax, std::abs(accel->GetBodyAccel(1)));
      max_ay = std::max(max_ay, std::abs(accel->GetBodyAccel(2)));
      max_az = std::max(max_az, std::abs(accel->GetBodyAccel(3)));
    }

    TS_ASSERT(std::isfinite(max_ax));
    TS_ASSERT(std::isfinite(max_ay));
    TS_ASSERT(std::isfinite(max_az));
  }

  // Test UVW dot (velocity derivatives)
  void testC172xUVWDot() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
  ...
  // Test acceleration response to aileron input
  void testC172xAccelerationFromAileron() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
  ...
  // Test gravity components at different attitudes
  void testC172xGravityComponentsAtAttitude() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    // Climb to get off ground
    fcs->SetDeCmd(-0.3);

    for (int i = 0; i < 500; i++) fdmex.Run();

    double grav_x = pm->GetNode("accelerations/gravity-x-ft_sec2", true)->getDoubleValue();
    double grav_y = pm->GetNode("accelerations/gravity-y-ft_sec2", true)->getDoubleValue();
    double grav_z = pm->GetNode("accelerations/gravity-z-ft_sec2", true)->getDoubleValue();

    TS_ASSERT(std::isfinite(grav_x));
    TS_ASSERT(std::isfinite(grav_y));
    TS_ASSERT(std::isfinite(grav_z));
  }

  // Test acceleration data consistency
  void testC172xAccelerationDataConsistency() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    // Get via FGAccelerations
    double udot_accel = accel->GetUVWdot(1);

    // Get via property tree
    double udot_pm = pm->GetNode("accelerations/udot-ft_sec2", true)->getDoubleValue();

    TS_ASSERT(std::isfinite(udot_accel));
    TS_ASSERT(std::isfinite(udot_pm));
    // Both should be consistent
    TS_ASSERT_DELTA(udot_accel, udot_pm, 0.001);
  }

  // Test acceleration values remain finite over time
  void testC172xAccelerationFiniteOverTime() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
  ...
  // Test Nx, Ny, Nz load factors
  void testC172xLoadFactors() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
  ...
  // Test acceleration during ground roll
  void testC172xAccelerationGroundRoll() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    for (int i = 0; i < 200; i++) {
      fdmex.Run();
      max_ax = std::max(max_ax, accel->GetBodyAccel(1));
    }

    TS_ASSERT(std::isfinite(max_ax));
    // Should have positive forward acceleration
    TS_ASSERT(max_ax > 0.0);
  }

  // Test total acceleration magnitude
  void testC172xTotalAccelerationMagnitude() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
  ...
  // Test accelerations in NED frame
  void testC172xNEDAccelerations() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    double n_dot = pm->GetNode("accelerations/n-dot-ft_sec2", true)->getDoubleValue();
    double e_dot = pm->GetNode("accelerations/e-dot-ft_sec2", true)->getDoubleValue();
    double d_dot = pm->GetNode("accelerations/d-dot-ft_sec2", true)->getDoubleValue();

    TS_ASSERT(std::isfinite(n_dot));
    TS_ASSERT(std::isfinite(e_dot));
    TS_ASSERT(std::isfinite(d_dot));
  }
};
