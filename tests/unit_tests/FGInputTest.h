...
// ============================================================================
// C172x Integration Tests for FGInput
// ============================================================================

class FGInputC172xTest : public CxxTest::TestSuite
{
private:
...
public:
  ...
  // Test 1: FGInput access with C172x model
  void testC172xInputAccess() {
    auto input = fdm.GetInput();
    TS_ASSERT(input != nullptr);
  }

  // Test 2: FGInput InitModel with C172x
  void testC172xInputInitModel() {
    auto input = fdm.GetInput();
    bool result = input->InitModel();
    TS_ASSERT(result == true || result == false);
  }

  // Test 3: FGInput Run with C172x
  void testC172xInputRun() {
    fdm.RunIC();
    auto input = fdm.GetInput();
    bool result = input->Run(false);
    TS_ASSERT(result == true || result == false);
  ...
  // Test 4: FGInput Run in holding mode with C172x
  void testC172xInputRunHolding() {
    fdm.RunIC();
    auto input = fdm.GetInput();
    bool result = input->Run(true);
    TS_ASSERT(result == true || result == false);
  ...
  // Test 5: FGInput Enable/Disable with C172x
  void testC172xInputEnableDisable() {
    auto input = fdm.GetInput();
    input->Enable();
    input->Disable();
    TS_ASSERT(true);
  ...
  // Test 6: C172x property path validation via throttle
  void testC172xPropertyPathValidation() {
    fdm.RunIC();
    std::string throttlePath = "fcs/throttle-cmd-norm";
    bool hasSlash = throttlePath.find('/') != std::string::npos;
    TS_ASSERT(hasSlash);

    size_t slashPos = throttlePath.find('/');
    std::string category = throttlePath.substr(0, slashPos);
    TS_ASSERT_EQUALS(category, "fcs");
  }

  // Test 7: C172x property value setting via PropertyManager
  void testC172xPropertyValueSetting() {
    fdm.RunIC();
  ...
  // Test 8: C172x property value getting via PropertyManager
  void testC172xPropertyValueGetting() {
    fdm.RunIC();
    fdm.Run();
  ...
  // Test 9: C172x input multiple property sets
  void testC172xMultiplePropertySets() {
    fdm.RunIC();
    auto pm = fdm.GetPropertyManager();
  ...
  // Test 10: C172x input zero value handling
  void testC172xInputZeroValue() {
    fdm.RunIC();
    auto pm = fdm.GetPropertyManager();
  ...
  // Test 11: C172x input after simulation run
  void testC172xInputAfterSimRun() {
    fdm.RunIC();
  ...
  // Test 12: C172x input stability over time
  void testC172xInputStabilityOverTime() {
    fdm.RunIC();
    auto pm = fdm.GetPropertyManager();
