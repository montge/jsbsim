...
// ============ C172x Aircraft Atmospheric Model Integration Tests ============
class FGAtmosphericModelC172xTest : public CxxTest::TestSuite
{
public:

  // Test C172x atmosphere model initialization
  void testC172xAtmosphereInitialization() {
    FGFDMExec fdmex;
  ...
  // Test C172x ISA temperature lapse with altitude
  void testC172xTemperatureLapseWithAltitude() {
    double temps[3];
    ...
    for (int j = 0; j < 3; j++) {
      ...
      temps[j] = fdmex.GetAtmosphere()->GetTemperature();
  ...
  // Test C172x pressure decrease with altitude
  void testC172xPressureDecreaseWithAltitude() {
    double pressures[3];
    double alts[] = {0.0, 5000.0, 10000.0};
    ...
    for (int j = 0; j < 3; j++) {
      ...
      pressures[j] = fdmex.GetAtmosphere()->GetPressure();
      TS_ASSERT(std::isfinite(pressures[j]));
      TS_ASSERT(pressures[j] > 0.0);
    }

    // Pressure should decrease with altitude
    TS_ASSERT(pressures[0] > pressures[1]);
    TS_ASSERT(pressures[1] > pressures[2]);
  }

  // Test C172x density decrease with altitude
  void testC172xDensityDecreaseWithAltitude() {
    double densities[3];
    double alts[] = {0.0, 5000.0, 10000.0};
    ...
    for (int j = 0; j < 3; j++) {
      ...
      densities[j] = fdmex.GetAtmosphere()->GetDensity();
      TS_ASSERT(std::isfinite(densities[j]));
      TS_ASSERT(densities[j] > 0.0);
    }

    // Density should decrease with altitude
    TS_ASSERT(densities[0] > densities[1]);
  ...
  // Test C172x speed of sound calculation
  void testC172xSpeedOfSoundCalculation() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
  ...
  // Test C172x sea level standard atmosphere
  void testC172xSeaLevelStandardAtmosphere() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    ...
    // Check close to ISA sea level values
    double tempK = temp / 1.8;
    TS_ASSERT(std::abs(tempK - 288.15) < 5.0);  // ~288K
  }

  // Test C172x atmosphere consistency during flight
  void testC172xAtmosphereConsistencyDuringFlight() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    ...
    double prevDens = atm->GetDensity();
    ...
    for (int i = 0; i < 100; i++) {
      ...
      // Values should be consistent (not changing wildly)
      if (i > 10) {
        TS_ASSERT(std::abs(temp - prevTemp) < 50.0);
        TS_ASSERT(std::abs(dens - prevDens) < 0.001);
      }
  ...
  // Test C172x flight at high altitude
  void testC172xFlightAtHighAltitude() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
  ...
  // Test C172x kinematic viscosity
