...
//=============================================================================
// C172x Integration Tests - Electric Motor Context Tests (C172x uses piston)
//=============================================================================

class FGElectricMotorC172xTest : public CxxTest::TestSuite
{
private:
...
public:
  ...
  // Test 1: C172x engine power output
  void testC172xEnginePower() {
    auto ic = fdm.GetIC();
    ...
    TS_ASSERT(power > 0.0 || propulsion->GetEngine(0)->GetThrust() > 0.0);
  }

  // Test 2: Throttle response linearity
  void testThrottleResponseLinearity() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
    ...
    fcs->SetMixtureCmd(-1, 1.0);
    for (int i = 0; i < 50; ++i) fdm.Run();
    double thrust30 = propulsion->GetEngine(0)->GetThrust();
    ...
    for (int i = 0; i < 50; ++i) fdm.Run();
    double thrust90 = propulsion->GetEngine(0)->GetThrust();

    TS_ASSERT(thrust60 > thrust30);
    TS_ASSERT(thrust90 > thrust60);
  }

  // Test 3: Engine torque effect on aircraft
  void testEngineTorqueEffect() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
  ...
  // Test 4: Propeller RPM tracking
  void testPropellerRPMTracking() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
  ...
  // Test 5: Power at different altitudes
  void testPowerAtAltitudes() {
    auto fcs = fdm.GetFCS();
    auto propulsion = fdm.GetPropulsion();
    auto ic = fdm.GetIC();
    ...
    fdm.RunIC();
    for (int i = 0; i < 50; ++i) fdm.Run();
    double thrust_10k = propulsion->GetEngine(0)->GetThrust();

    TS_ASSERT(std::isfinite(thrust_sl));
    TS_ASSERT(std::isfinite(thrust_10k));
  }

  // Test 6: Fuel flow at cruise
  void testFuelFlowAtCruise() {
    auto ic = fdm.GetIC();
  ...
  // Test 7: Engine running state
  void testEngineRunningState() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
  ...
  // Test 8: Idle power setting
  void testIdlePowerSetting() {
    auto ic = fdm.GetIC();
    ...
    auto fcs = fdm.GetFCS();
    fcs->SetThrottleCmd(-1, 0.1);  // Near idle
    fcs->SetMixtureCmd(-1, 1.0);
  ...
  // Test 9: Full power takeoff
  void testFullPowerTakeoff() {
    auto ic = fdm.GetIC();
  ...
  // Test 10: Engine cutoff
  void testEngineCutoff() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
  ...
  // Test 11: Power available is non-negative
  void testPowerNonNegative() {
    ...
    for (double throttle = 0.0; throttle <= 1.0; throttle += 0.25) {
      ...
      double power = propulsion->GetEngine(0)->GetPowerAvailable();
      TS_ASSERT(power >= 0.0 || std::isfinite(power));
    }
  }

  // Test 12: Extended run stability
  void testExtendedRunStability() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
