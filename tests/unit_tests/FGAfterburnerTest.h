...
//=============================================================================
// C172x Integration Tests - Propulsion Tests (Piston Engine, not Afterburner)
//=============================================================================

class FGAfterburnerC172xTest : public CxxTest::TestSuite
{
private:
...
public:
  ...
  // Test 1: C172x engine produces thrust at full throttle
  void testC172xEngineThrust() {
    auto ic = fdm.GetIC();
    ...
    fcs->SetMixtureCmd(-1, 1.0);   // Rich mixture
  ...
  // Test 2: Thrust increases with throttle
  void testThrustIncreasesWithThrottle() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
    ...
    for (int i = 0; i < 50; ++i) fdm.Run();
    double thrust_low = propulsion->GetEngine(0)->GetThrust();

    // High throttle
    fcs->SetThrottleCmd(-1, 0.9);
    for (int i = 0; i < 50; ++i) fdm.Run();
    double thrust_high = propulsion->GetEngine(0)->GetThrust();

    TS_ASSERT(thrust_high > thrust_low);
  }

  // Test 3: Fuel flow increases with throttle
  void testFuelFlowIncreasesWithThrottle() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
    ...
    fcs->SetMixtureCmd(-1, 1.0);
    for (int i = 0; i < 50; ++i) fdm.Run();
    double ff_low = propulsion->GetEngine(0)->GetFuelFlowRate();

    fcs->SetThrottleCmd(-1, 0.9);
    for (int i = 0; i < 50; ++i) fdm.Run();
    double ff_high = propulsion->GetEngine(0)->GetFuelFlowRate();

    TS_ASSERT(ff_high > ff_low);
  }

  // Test 4: Engine RPM responds to throttle
  void testEngineRPMRespondsToThrottle() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
  ...
  void testPropellerEfficiency() {
    ...
    TS_ASSERT(thrust > 0.0);
    TS_ASSERT(power > 0.0 || thrust > 0.0);  // Power or thrust available
  }

  // Test 6: Altitude affects engine performance
  void testAltitudeAffectsPerformance() {
    auto fcs = fdm.GetFCS();
    auto propulsion = fdm.GetPropulsion();
    auto ic = fdm.GetIC();
    ...
    fcs->SetMixtureCmd(-1, 1.0);
    for (int i = 0; i < 50; ++i) fdm.Run();
    double thrust_sl = propulsion->GetEngine(0)->GetThrust();
    ...
    TS_ASSERT(thrust_sl >= thrust_high * 0.5);  // At least half the thrust
  }

  // Test 7: Mixture affects performance
  void testMixtureAffectsPerformance() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
    ...
    fcs->SetMixtureCmd(-1, 1.0);
    for (int i = 0; i < 50; ++i) fdm.Run();
    double thrust_rich = propulsion->GetEngine(0)->GetThrust();

    // Lean mixture
    fcs->SetMixtureCmd(-1, 0.5);
    for (int i = 0; i < 50; ++i) fdm.Run();
    double thrust_lean = propulsion->GetEngine(0)->GetThrust();

    // Both should be finite
    TS_ASSERT(std::isfinite(thrust_rich));
    TS_ASSERT(std::isfinite(thrust_lean));
  }

  // Test 8: Engine starts properly
  void testEngineStarts() {
    ...
    auto propulsion = fdm.GetPropulsion();
    bool running = propulsion->GetEngine(0)->GetRunning();

    // Engine should be running (assuming model starts running)
    TS_ASSERT(running || propulsion->GetEngine(0)->GetThrust() >= 0.0);
  }

  // Test 9: Thrust is finite at all conditions
  void testThrustAlwaysFinite() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
    ...
    // Test multiple throttle settings
    for (double throttle = 0.0; throttle <= 1.0; throttle += 0.2) {
  ...
  // Test 10: Power available is finite
  void testPowerAlwaysFinite() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
  ...
  // Test 11: Fuel consumption accumulates
  void testFuelConsumption() {
    ...
    for (int i = 0; i < 100; ++i) {
      fdm.Run();
      double fuelFlow = propulsion->GetEngine(0)->GetFuelFlowRate();
