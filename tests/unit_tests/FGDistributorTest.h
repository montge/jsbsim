...
/*******************************************************************************
 * C172x Integration Tests for FGDistributor
 *
 * Tests distributor behavior in realistic flight scenarios using the C172x.
 * Verifies conditional output distribution in FCS components.
 ******************************************************************************/
class FGDistributorC172xTest : public CxxTest::TestSuite
{
public:
  // Test 1: C172x FCS loads successfully
  ...
  // Test 2: Roll command distribution between autopilot and manual
  void testC172xRollCommandDistribution() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    ...
    double rollCmd = fdmex.GetPropertyValue("fcs/roll-trim-sum");
    TS_ASSERT(std::isfinite(rollCmd));
  }

  // Test 3: Pitch control distribution
  void testC172xPitchControlDistribution() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
    ...
    double pitchSum = fdmex.GetPropertyValue("fcs/pitch-trim-sum");
    TS_ASSERT(std::isfinite(pitchSum));
  }

  // Test 4: Yaw control distribution
  void testC172xYawControlDistribution() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
    ...
    double yawSum = fdmex.GetPropertyValue("fcs/yaw-trim-sum");
    TS_ASSERT(std::isfinite(yawSum));
  }

  // Test 5: Autopilot mode distribution
  void testC172xAutopilotModeDistribution() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    ...
    double hdgCmd = fdmex.GetPropertyValue("fcs/heading-command");
    TS_ASSERT(std::isfinite(hdgCmd));
  }

  // Test 6: Throttle distribution to engine
  void testC172xThrottleDistribution() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
    ...
    double throttlePos = fdmex.GetPropertyValue("fcs/throttle-pos-norm");
    TS_ASSERT(std::isfinite(throttlePos));
    TS_ASSERT_DELTA(throttlePos, 0.75, 0.1);
  }

  // Test 7: Brake distribution left/right
  void testC172xBrakeDistribution() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
    ...
    TS_ASSERT(lBrake > rBrake);
  }

  // Test 8: Control surface position distribution
  void testC172xControlSurfaceDistribution() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
    ...
    // Ailerons should move in opposite directions
    double leftAileron = fdmex.GetPropertyValue("fcs/left-aileron-pos-rad");
    double rightAileron = fdmex.GetPropertyValue("fcs/right-aileron-pos-rad");

    TS_ASSERT(std::isfinite(leftAileron));
    TS_ASSERT(std::isfinite(rightAileron));
  }

  // Test 9: Mixture distribution
  void testC172xMixtureDistribution() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
  ...
  // Test 10: Flap position distribution
  void testC172xFlapDistribution() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
  ...
  // Test 11: Multiple input sources combined
  void testC172xMultipleInputSources() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
  ...
  // Test 12: Ground steering distribution
  void testC172xGroundSteeringDistribution() {
    FGFDMExec fdmex;
    TS_ASSERT(fdmex.LoadModel("c172x"));
    TS_ASSERT(fdmex.RunIC());
    ...
    double steerCmd = fdmex.GetPropertyValue("fcs/steer-cmd-norm");
    TS_ASSERT(std::isfinite(steerCmd));
  }
};
