...
/*******************************************************************************
 * C172x Integration Tests for Atmosphere Model
 ******************************************************************************/

class FGStandardAtmosphereC172xTest : public CxxTest::TestSuite
{
public:
  // Test atmosphere temperature from C172x
  void testC172xAtmosphereTemperature() {
    FGFDMExec fdmex;
  ...
  // Test atmosphere pressure from C172x
  void testC172xAtmospherePressure() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
  ...
  // Test atmosphere density from C172x
  void testC172xAtmosphereDensity() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
  ...
  // Test speed of sound from C172x
  void testC172xSpeedOfSound() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    double a = atm->GetSoundSpeed();
    TS_ASSERT(std::isfinite(a));
    TS_ASSERT(a > 0.0);
    // Speed of sound at sea level ~1100 fps
    TS_ASSERT(a > 900.0 && a < 1200.0);
  }

  // Test atmosphere at altitude during climb
  void testC172xAtmosphereDuringClimb() {
    ...
    double initial_temp = atm->GetTemperature();
    double initial_press = atm->GetPressure();
    ...
    double final_temp = atm->GetTemperature();
    double final_press = atm->GetPressure();

    TS_ASSERT(std::isfinite(final_temp));
    TS_ASSERT(std::isfinite(final_press));
    // If we climbed, temp and pressure should decrease
    if (final_alt > initial_alt + 100) {
      TS_ASSERT(final_temp <= initial_temp);
      TS_ASSERT(final_press <= initial_press);
    }
  }

  // Test atmosphere properties via property tree
  void testC172xAtmosphereViaPropertyTree() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    double temp = pm->GetNode("atmosphere/T-R", true)->getDoubleValue();
    double press = pm->GetNode("atmosphere/P-psf", true)->getDoubleValue();
    double rho = pm->GetNode("atmosphere/rho-slugs_ft3", true)->getDoubleValue();
  ...
  // Test density altitude from C172x
  ...
  // Test pressure altitude from C172x
  void testC172xPressureAltitude() {
    ...
    TS_ASSERT(std::isfinite(pa));
  }

  // Test atmosphere data consistency
  void testC172xAtmosphereDataConsistency() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    // Get via FGAtmosphere
    double temp_atm = atm->GetTemperature();

    // Get via property tree
    double temp_pm = pm->GetNode("atmosphere/T-R", true)->getDoubleValue();

    TS_ASSERT(std::isfinite(temp_atm));
    TS_ASSERT(std::isfinite(temp_pm));
    TS_ASSERT_DELTA(temp_atm, temp_pm, 0.1);
  }

  // Test atmosphere values remain finite over time
  void testC172xAtmosphereFiniteOverTime() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    for (int i = 0; i < 500; i++) {
      ...
      double press = atm->GetPressure();
      double rho = atm->GetDensity();
      if (!std::isfinite(temp) || !std::isfinite(press) || !std::isfinite(rho)) {
        all_finite = false;
        break;
      }
    }
  ...
  // Test viscosity from C172x atmosphere
  void testC172xAtmosphereViscosity() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    TS_ASSERT(std::isfinite(mu));
    TS_ASSERT(std::isfinite(nu));
    TS_ASSERT(mu > 0.0);
    TS_ASSERT(nu > 0.0);
  }

  // Test sea level values from C172x
  void testC172xAtmosphereSeaLevelValues() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    double temp_sl = atm->GetTemperatureSL();
    double press_sl = atm->GetPressureSL();

    TS_ASSERT(std::isfinite(temp_sl));
    TS_ASSERT(std::isfinite(press_sl));
    TS_ASSERT(temp_sl > 0.0);
    TS_ASSERT(press_sl > 0.0);
  }

  // Test atmosphere stability during various maneuvers
  void testC172xAtmosphereStabilityDuringManeuvers() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    for (int cycle = 0; cycle < 5; cycle++) {
      ...
      for (int i = 0; i < 50; i++) {
        fdmex.Run();
        if (atm->GetTemperature() <= 0 || atm->GetPressure() <= 0) {
          all_valid = false;
        }
      }
  ...
  // Test standard day sound speed
  void testC172xStandardDaySoundSpeed() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    fdmex.RunIC();
    ...
    double a_sl = atm->StdDaySLsoundspeed;
    TS_ASSERT(std::isfinite(a_sl));
    TS_ASSERT(a_sl > 1000.0);  // Should be around 1116 fps
  }

  // Test sigma (density ratio)
  void testC172xDensityRatio() {
    ...
    double sigma = pm->GetNode("atmosphere/sigma", true)->getDoubleValue();
    TS_ASSERT(std::isfinite(sigma));
    TS_ASSERT(sigma > 0.0);
    TS_ASSERT(sigma <= 1.1);  // At or below sea level
  }
};
