...
//=============================================================================
// C172x Integration Tests - Aerodynamic Body Axis Tests with Real Aircraft
//=============================================================================

class FGAeroBodyC172xTest : public CxxTest::TestSuite
{
private:
...
public:
  ...
  // Test 1: C172x produces lift in flight
  void testC172xProducesLift() {
    auto ic = fdm.GetIC();
    ...
    double liftForce = aero->GetForces()(3);  // Z-axis body force

    TS_ASSERT(liftForce < 0.0);  // Negative Z is up in body axes
  }

  // Test 2: C172x drag increases with speed
  void testC172xDragIncreasesWithSpeed() {
    auto ic = fdm.GetIC();
    ic->SetAltitudeASLFtIC(5000.0);
    ...
    for (int i = 0; i < 10; ++i) fdm.Run();
    auto aero = fdm.GetAerodynamics();
    double drag80 = -aero->GetForces()(1);  // Negative X is drag

    // Then at 120 knots
    ic->SetVcalibratedKtsIC(120.0);
    fdm.RunIC();
    for (int i = 0; i < 10; ++i) fdm.Run();
    double drag120 = -aero->GetForces()(1);

    TS_ASSERT(drag120 > drag80);
  }

  // Test 3: Elevator deflection changes pitching moment
  void testElevatorPitchingMoment() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
    ...
    fcs->SetDeCmd(0.0);
    for (int i = 0; i < 10; ++i) fdm.Run();
    double moment_neutral = aero->GetMoments()(2);

    // Elevator up (nose up)
    fcs->SetDeCmd(-0.5);
    for (int i = 0; i < 10; ++i) fdm.Run();
    double moment_up = aero->GetMoments()(2);

    TS_ASSERT(moment_up != moment_neutral);
  }

  // Test 4: Aileron deflection creates roll moment
  void testAileronRollMoment() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
  ...
  // Test 5: Rudder deflection creates yaw moment
  void testRudderYawMoment() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
    ...
    fcs->SetDrCmd(0.5);
    for (int i = 0; i < 10; ++i) fdm.Run();
    double yawMoment = aero->GetMoments()(3);

    TS_ASSERT(yawMoment != 0.0);
  }

  // Test 6: Dynamic pressure affects forces
  ...
  // Test 7: Angle of attack affects lift
  void testAOALiftRelation() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    ...
    for (int i = 0; i < 10; ++i) fdm.Run();
    auto aero = fdm.GetAerodynamics();
    double lift_low_aoa = -aero->GetForces()(3);

    // Higher AOA
    ic->SetAlphaDegIC(8.0);
    fdm.RunIC();
    for (int i = 0; i < 10; ++i) fdm.Run();
    double lift_high_aoa = -aero->GetForces()(3);

    TS_ASSERT(lift_high_aoa > lift_low_aoa);
  }

  // Test 8: Sideslip creates side force
  void testSideslipSideForce() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    ic->SetBetaDegIC(5.0);
    ...
    auto aero = fdm.GetAerodynamics();
    double sideForce = aero->GetForces()(2);

    TS_ASSERT(sideForce != 0.0);
  }

  // Test 9: Body axis forces are finite
  void testBodyAxisForcesFinite() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
    ...
    auto aero = fdm.GetAerodynamics();
    const auto& forces = aero->GetForces();
  ...
  // Test 10: Body axis moments are finite
  void testBodyAxisMomentsFinite() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
  ...
  // Test 11: Stability axis coefficients
  void testStabilityAxisCoefficients() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
  ...
  void testExtendedSimulationAeroState() {
    auto ic = fdm.GetIC();
    ic->SetVcalibratedKtsIC(100.0);
    ic->SetAltitudeASLFtIC(5000.0);
    fdm.RunIC();
