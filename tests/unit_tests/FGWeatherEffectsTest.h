...
// ============ C172x Aircraft Weather Effects Integration Tests ============
class FGWeatherEffectsC172xTest : public CxxTest::TestSuite
{
public:

  // Test C172x atmospheric properties access
  void testC172xAtmosphereAccess() {
    FGFDMExec fdmex;
  ...
  void testC172xDensityAltitude() {
    ...
    TS_ASSERT(densityAlt < 50000.0);
  }

  // Test C172x flight at different altitudes
  void testC172xFlightAtVariousAltitudes() {
  ...
  // Test C172x TAS vs CAS relationship
  void testC172xTASvsCAS() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    ...
    double vtas = aux->GetVtrueFPS() * 0.592484;  // Convert fps to kts

    TS_ASSERT(std::isfinite(vcas));
    TS_ASSERT(std::isfinite(vtas));
    // At altitude, TAS > CAS
    TS_ASSERT(vtas >= vcas * 0.9);  // Some tolerance
  }
  ...
  // Test C172x temperature at altitude
  void testC172xTemperatureAtAltitude() {
    ...
    double tempK = tempR / 1.8;  // Convert Rankine to Kelvin

    TS_ASSERT(std::isfinite(tempR));
    // Temperature should decrease with altitude
    TS_ASSERT(tempK < 288.15);  // Less than sea level standard
    TS_ASSERT(tempK > 200.0);   // But still reasonable
  }

  // Test C172x pressure at altitude
  void testC172xPressureAtAltitude() {
    ...
    auto atm = fdmex.GetAtmosphere();
    double pressure = atm->GetPressure();  // psf

    TS_ASSERT(std::isfinite(pressure));
    // Pressure should be less than sea level
    TS_ASSERT(pressure > 0.0);
    TS_ASSERT(pressure < 2116.22);  // Less than sea level standard (psf)
  }

  // Test C172x Mach number calculation
  void testC172xMachNumber() {
    ...
    TS_ASSERT(mach > 0.0);
    TS_ASSERT(mach < 0.5);  // C172x is subsonic
  }

  // Test C172x flight during simulation with weather model
  void testC172xFlightWithAtmosphere() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
  ...
  // Test C172x engine performance vs altitude
  void testC172xEnginePerformanceVsAltitude() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    ...
    double powerLowAlt = prop->GetEngine(0)->GetPowerAvailable();

    // Now test at higher altitude
    FGFDMExec fdmex2;
    fdmex2.LoadModel("c172x");

    auto ic2 = fdmex2.GetIC();
    ic2->SetAltitudeASLFtIC(10000.0);
    ...
    fcs2->SetThrottleCmd(-1, 1.0);

    for (int i = 0; i < 100; i++) {
      fdmex2.Run();
    }

    double powerHighAlt = prop2->GetEngine(0)->GetPowerAvailable();

    TS_ASSERT(std::isfinite(powerLowAlt));
    TS_ASSERT(std::isfinite(powerHighAlt));
  }

  // Test C172x atmospheric stability during flight
  void testC172xAtmosphericStabilityDuringFlight() {
    FGFDMExec fdmex;
    fdmex.LoadModel("c172x");
    ...
    fcs->SetDeCmd(-0.1);  // Slight pitch up
